  ...
  return function()
    ...
    for _, client in pairs(vim.lsp.buf_get_clients(0)) do
      if client.name == "null-ls" and expand_null_ls then
        vim.list_extend(buf_client_names, doomnvim.null_ls_sources(vim.bo.filetype, "FORMATTING"))
        vim.list_extend(buf_client_names, doomnvim.null_ls_sources(vim.bo.filetype, "DIAGNOSTICS"))
      else
...
function M.provider.treesitter_status()
  ...
  return (ts and next(ts)) and " TS" or ""
end
